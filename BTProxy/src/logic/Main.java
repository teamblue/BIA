...
public class Main {
	...
	private static void checkNotify() {
		...
		if (notifiedKB == false) {	
			if (notifyKB > -1) {
				if (totalKB >= notifyKB) {
					notifiedKB = true;				
					addEvent("User notification: bandwidth limit of " + notifyKB + " KB reached");	
					displayNotification("Bandwidth limit of " + notifyKB + " KB reached");
				}
			}
		}
		...
		if (notifiedCost == false) {
			if (notifyCost > -1) {
				if ( (totalKB * costPerKB) >= notifyCost) {
					notifiedCost = true;			
					addEvent("User notification: cost limit of $" + notifyCost + " reached");	
					displayNotification("Cost limit of $" + notifyCost + " reached");
				}
			}
		}
	...
	private static void displayNotification(String message) {
		...
		MessageBox messageBox = new MessageBox(shell, SWT.ICON_WARNING | SWT.OK | SWT.SYSTEM_MODAL);
	...
	public static void addEvent(String event) {
        ...
        //debug window is open so let's update the debug window's list box and the events array
			...
			//SWT is not thread safe. This prevents an invalid thread access exception due
	...
	public static void setupTrayIcon() throws Exception {
		...
		//this while loop is required for SWT and is why we need a separate thread
